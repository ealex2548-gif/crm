...
import{ArrowLeft,MessageCircle,Phone,Monitor,Ticket,X}from"lucide-react";
...
export function ClientDetailsPanel({active,mobile,setMobile,tab,setTab,setPage,ticketsPageTarget="tickets",onUpdate,onClose}){
return <aside className={"details "+(mobile==="details"?"mobile-show":"")}>
<div className="details-head"><button className="mobile-back" onClick={()=>setMobile("chat")}><ArrowLeft/></button><strong>Dados do cliente</strong><button className="details-close" title="Fechar" onClick={onClose}><X/></button></div><div className="profile"><Avatar c={active}/><h2>{active.name}</h2><p>{active.phone} · {active.company}</p></div><div className="quick"><button><MessageCircle/>Mensagem</button><button><Phone/>Ligar</button><button><Monitor/>Remoto</button><button onClick={()=>FEATURES.tickets&&setPage(ticketsPageTarget)}><Ticket/>Chamados</button></div>

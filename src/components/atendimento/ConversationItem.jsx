...
<Avatar c={c}/><div className="conv-main"><div className="conv-top"><strong>{c.name}</strong><span className={c.unread>0?"unread":""}>{c.time}</span></div><div className="preview"><span>{c.preview}</span>{c.unread>0&&<b>{c.unread}</b>}{onMarkRead&&<button type="button" className="conv-menu-btn" title="Mais opções" onClick={e=>{e.stopPropagation();setMenu(v=>!v)}}><ChevronDown/></button>}</div><div className="waitline"><Clock3/> esperando {c.waiting}{c.hasNote&&<span>• nota interna</span>}</div><div className="tags"><i>{c.sector}</i><i className={(c.priority==="Alta"||c.priority==="Urgente")?"danger":"warning"}>{c.priority}</i>{c.status==="Aguardando aceite"?<i className="waiting-tag">Aguardando aceite · {c.agent.split(" ")[0]}</i>:c.assignedAgentId?<i className="muted">{c.agent.split(" ")[0]}</i>:c.status!=="Finalizado"&&<i className="waiting-tag">Aguardando</i>}<i className={"sla "+(c.slaStatus==="breached"?"sla-breached":"")}>SLA {c.sla}</i></div></div>
